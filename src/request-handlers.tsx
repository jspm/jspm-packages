...
const staticResources = {
  ...
  "/example-browser-input.html": "./src/example-browser.html",
  "/example-browser-output.html": "./lib/example-browser.html",
...
async function renderPackagePage(request: Request, packageNameVersion: {
  name: string;
  version: string;
}) {
  try {
    const templateURL = new URL("../lib/package.html", import.meta.url);
    const templateFileResponse = await fetch(templateURL);
...
async function getPackageComponentProps(packageNameVersion: {
  ...
  try {
    const baseURL =
      `${NPM_PROVIDER_URL}${packageNameVersion.name}@${packageNameVersion.version}`;
...
async function requestHandlerSearch(request: Request) {
  try {
    const templateURL = new URL("../lib/search.html", import.meta.url);
    const templateFileResponse = await fetch(templateURL);
